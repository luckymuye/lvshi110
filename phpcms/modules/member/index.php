...
class index extends foreground {
	 ...
	 * 该函数为QQ登录回调地址
	 */
	public function public_qq_loginnew2(){
					...
					if(!empty($openid)){
						$r = $this->db->get_one(array('connectid'=>$openid,'from'=>'qq'));
						 ...
						 if(!empty($r)){
								...
								$cookietime = $_cookietime ? TIME + $_cookietime : 0;
								$phpcms_auth = sys_auth($userid."\t".$password, 'ENCODE', get_auth_key('login'));
								$userinfos = $info->get_user_info2();
								param::set_cookie('auth', $phpcms_auth, $cookietime);
								param::set_cookie('_userid', $userid, $cookietime);
								param::set_cookie('_username', $username, $cookietime);
								param::set_cookie('_nickname', $userinfos->nickname, $cookietime);
								param::set_cookie('_userpic', $userinfos->figureurl, $cookietime);
								param::set_cookie('_groupid', $groupid, $cookietime);
								param::set_cookie('cookietime', $_cookietime, $cookietime);
								param::set_cookie('_nickname', $nickname, $cookietime);
