...
class liuyan_admin extends admin {
	private $liuyan_db,$siteid;
	function __construct() {
		parent::__construct();
		$this->M=new_html_special_chars(getcache('liuyan', 'commons'));//读取留言本配置缓存文件
		$this->liuyan_db = pc_base::load_model('liuyan_model');
		$this->siteid = $this->get_siteid();
	}
	
	public function init() {
		if($_GET['ispass']=='no'){//显示未回复  ispass 0表示未回复 1表示已回复，2表示不显示
			$where = array('ispass'=>'0');
		}
		$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
		$infos = $this->liuyan_db->listinfo($where,$order = 'lydate DESC',$page, $pages = '10');
